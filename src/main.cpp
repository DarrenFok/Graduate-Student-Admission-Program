...
//   //input internationalStudents
    ...
    while (getline(internationalFile, line)){
        istringstream ss(line);
        string firstName, lastName, country, s_cgpa, s_researchScore, reading, listening, speaking, writing, province;
        float cgpa;
        int researchScore;
        ...
        if(score.getReading() > 19 && score.getListening() > 19 && score.getSpeaking() > 19 &&
           score.getWriting() > 19 && score.getTotal() > 92) {
            MLI.sortedMerge(firstName, lastName, cgpa, researchScore, MLI.getIntID(), country, score, province);
        }
        mergeCount++;
    ...
    while (getline(domesticFile, line)) {
        ...
        toefl mscore;
        //mscore.setReading(stoi(reading));
        //mscore.setListening(stoi(listening));
        //mscore.setSpeaking(stoi(speaking));
        //mscore.setWriting(stoi(writing));
        mscore.setTotal();

        cgpa = stof(s_cgpa);
        researchScore = stoi(s_researchScore);

        MLI.sortedMerge(firstName, lastName, cgpa, researchScore, MLI.getIntID(), country, mscore, province);
        mergeCount++;
